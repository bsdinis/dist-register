...
impl ServerUniverse {
    pub proof fn dummy() -> (tracked r: Self)
        ensures
            r.inv(),
            forall |q: Quorum| r.valid_quorum(q) ==> r.quorum_timestamp(q) >= (Timestamp { seqno: 0, client_id: 0}),
    {
            ...
            r.0.len() <= q@.len(),
    {
        ...
        assert(vals.finite()) by {
            ...
            self.map.restrict(q@).values().lemma_map_finite(|r: Tracked<MonotonicTimestampResource>| r@@.timestamp());
        }
        assume(lbs.finite());
    ...
    proof fn lemma_quorum_timestamp_is_upper_bound(self, q: Quorum)
        requires
            ...
            self.valid_quorum(q),
        ensures
            forall |idx: nat| q@.contains(idx) ==> self.quorum_timestamp(q) >= self[idx]@@.timestamp(),
    {
        let ts_leq = Self::ts_leq();
            ...
            forall |q: Quorum| other.valid_quorum(q) ==> other.quorum_timestamp(q) >= min,
    {
        assert forall |q: Quorum| other.valid_quorum(q) implies other.quorum_timestamp(q) >= min by {
            ...
            assert(forall |idx: nat| self.contains_key(idx) ==> other[idx]@@.timestamp() >= self[idx]@@.timestamp());
    ...
    {
        assert forall |q: Quorum| self.valid_quorum(q)
            implies self.quorum_timestamp(q) >= ts by {
    ...
    proof fn lemma_quorum_agree(self, q1: Quorum, q2: Quorum, lb: Timestamp)
        ...
        ensures
            forall |idx: nat| q1@.contains(idx) && q2@.contains(idx) ==> self[idx]@@.timestamp() >= lb
    {
        let restr = self.map.restrict(q1@.intersect(q2@));
        assert forall |idx: nat|
                q1@.contains(idx) && q2@.contains(idx)
            implies
                self[idx]@@.timestamp() >= lb
            by
        {
