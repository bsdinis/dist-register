...
impl<C> vstd::rwlock::RwLockPredicate<HashMap<u64, C>> for ChannelInv
    where C: Channel<Id = (u64, u64), R = Request, S = Response, K = ChannelInv>
{
    open spec fn inv(self, v: HashMap<u64, C>) -> bool {
        forall |client_id: u64| #[trigger] v@.contains_key(client_id) ==> {
            self == v@[client_id].constant()
        }
    }
}
